...
void OpusDecoderWrapper::Config(int sample_rate, int channels, int duration_ms) {
    if (audio_dec_ == nullptr) {
        ESP_LOGE(TAG, "Audio decoder is not create");
        return;
    }

    opus_decoder_init(audio_dec_, sample_rate, channels);
